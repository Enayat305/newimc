$(document).ready(function() {
    ...
    //Brands table
    var brands_table = $('#departments_table').DataTable({
        processing: true,
        serverSide: true,
        ajax: '/lab/departments',
        columnDefs: [{
            targets: 2,
            orderable: false,
            searchable: false,
    ...
    $(document).on('click', 'button.edit_department_button', function() {
        $('div.department_modal').load($(this).data('href'), function() {
            $(this).modal('show');
    ...
    if ($('textarea#test_description').length > 0) {
        tinymce.init({
    ...
    test_particulars_table = $('#test_particulars_table').DataTable({
        processing: true,
        serverSide: true,
        ajax: {
            url: '/lab/testparticular',
            data: function(d) {
    ...
    $(document).on('click', '.delete_particulars_button', function(e) {
        e.preventDefault();
        swal({
            title: LANG.sure,
            text: 'This Test Particulars Will Be Deleted',
            icon: 'warning',
            buttons: true,
            dangerMode: true,
        }).then(willDelete => {
            if (willDelete) {
                ...
                $.ajax({
                    ...
                    success: function(result) {
                        if (result.success == true) {
                            toastr.success(result.msg);
                            test_particulars_table.ajax.reload();
                        } else {
                            toastr.error(result.msg);
                        }
                    },
...
$(document).ready(function() {
    ...
    purchase_table = $('#test_table').DataTable({
        processing: true,
        serverSide: true,
        ajax: {
            url: '/lab/tests',
        ...
        columns: [
            { data: 'action', name: 'action', orderable: false, searchable: false },
            { data: 'name', name: 'tests.name' },
            { data: 'test_code', name: 'tests.test_code' },
            { data: 'department', name: 'd.name' },
            { data: 'sample_require', name: 'tests.sample_require' },
            { data: 'final_total', name: 'tests.final_total' },
            { data: 'carry_out', name: 'tests.carry_out' },
            { data: 'report_time_day', name: 'tests.report_time_day' }
    ...
    $(document).on(
        ...
        '#purchase_list_filter_location_id, \
                    #department_id, #purchase_list_filter_payment_status,\
...
function get_purchase_entry_row(product_id, variation_id) {
    if (product_id) {
        ...
        $.ajax({
            method: 'POST',
            url: '/lab/get_test_product_entry_row',
            dataType: 'html',
...
function update_grand_total() {
    ...
    __write_number($('input.total-amount'), grand_total, true);
...
$(document).on('change', 'input.more-test-amount', function() {
    var grand_total = __read_number($('input.payment-amount'), true);
    var more_test_amount = __read_number($('input.more-test-amount'), true);
    var bal = grand_total + more_test_amount;
    __write_number($('input.total-amount'), bal, true);
});
...
$(document).on('click', 'button#submit_purchase_form', function(e) {
    ...
    $('form#add_purchase_form').validate({
        rules: {
            test_code: {
                remote: {
                    url: '/lab/purchases/check_test_code',
                    type: 'post',
                    data: {
                        test_code: function() {
        ...
        messages: {
            test_code: {
                remote: 'Test code already exists',
            },
        },
    });
