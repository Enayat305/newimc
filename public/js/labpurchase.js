        ...
        $('#search_product')
            .autocomplete({
                source: function(request, response) {
                    $.getJSON(
                        '/lab/pu/get_products', { location_id: $('#location_id').val(), term: request.term },
                        response
                    );
                },
                minLength: 2,
...
function get_purchase_entry_row(product_id, variation_id) {
    if (product_id) {
        ...
        $.ajax({
            method: 'POST',
            url: '/lab/purchases/get_purchase_entry_row',
            dataType: 'html',
...
$(document).on('click', 'button#submit_purchase_form', function(e) {
    ...
    $('form#add_purchase_form').validate({
        rules: {
            ref_no: {
                remote: {
                    url: '/lab/purchases/check_ref_number',
                    type: 'post',
                    data: {
                        ref_no: function() {
                            return $('#ref_no').val();
