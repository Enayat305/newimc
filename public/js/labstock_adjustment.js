        ...
        $('#search_product_for_srock_adjustment')
            .autocomplete({
                source: function(request, response) {
                    $.getJSON(
                        '/lab/products/list', { location_id: $('#location_id').val(), term: request.term },
                        response
                    );
                },
                minLength: 2,
...
function stock_adjustment_product_row(variation_id) {
    ...
    $.ajax({
        method: 'POST',
        url: '/lab/stock-adjustments/get_product_row',
...
function get_stock_adjustment_details(rowData) {
    var div = $('<div/>')
        ...
        .text('Loading...');
    $.ajax({
        url: '/lab/stock-adjustments/' + rowData.DT_RowId,
        dataType: 'html',
