...
class TransactionUtil extends Util
{
    ...
    public function createSellTransaction($business_id, $input, $invoice_total, $user_id, $uf_data = true)
    {
        ...
        $transaction = Transaction::create([
            ...
            'ref_no' => '',
            'spicemen' => $input['spicemen'],
            'ref_by'=>!empty($input['ref_by']) ? $input['ref_by'] : null,
    ...
    public function updateSellTransaction($transaction_id, $business_id, $input, $invoice_total, $user_id, $uf_data = true, $change_invoice_number = true)
    {
        ...
        $final_total = $uf_data ? $this->num_uf($input['final_total']) : $input['final_total'];
        if($input['location_id']==2){
             $lab_total=$final_total;
            }
            else{
                $lab_total= $invoice_total['total_before_tax'];
            }
        $update_date = [
            'status' => $input['status'],
            'invoice_no' => $invoice_no,
        ...
        foreach ($payments as $payment) {
            ...
            if (!empty($payment['payment_id'])) {
                ...
                if ($payment_amount != 0) {
                    ...
                    if ($transaction->type == 'doctor_fee') {
                        $prefix_type = 'doctor_fee_payment';
                    }
        ...
        if ($transaction_type == 'sell' && $transaction->status == 'final') {
            ...
            if ($il->show_previous_bal == 1) {
                ...
                if (!empty($all_due)) {
                    ...
                    $output['previous_due']=$this->num_f($all_due-$due, $show_currency, $business_details);
    ...
    {   
        ...
        $query = Transaction::where('transactions.business_id', $business_id)
                    ->where('transactions.type', 'lab_sell')
...
//OpeningBAlance
 /**
     ...
     * @return array
     */
    public function getOpenPurchaseTotals($business_id, $start_date = null, $end_date = null, $location_id = null)
    {
        ...
        if ($permitted_locations != 'all') {
            $query->whereIn('transactions.location_id', $permitted_locations);
            $contacts->whereIn('transactions.location_id', $permitted_locations);
        }

        if (!empty($start_date) && !empty($end_date)) {
            $query->whereBetween(DB::raw('date(transaction_date)'), [$start_date, $end_date]);
            $contacts->whereBetween(DB::raw('date(transaction_date)'), [$start_date, $end_date]);
        }

        if (empty($start_date) && !empty($end_date)) {
            $query->whereDate('transaction_date', '<=', $end_date);
            $contacts->whereDate('transaction_date', '<=', $end_date);
        }

        //Filter by the location
        if (!empty($location_id)) {
            $query->where('transactions.location_id', $location_id);
            $contacts->where('transactions.location_id', $location_id);

        }

        $purchase_details = $query->get();
        $contact=$contacts->get();
        $contacts_balance_due=$contact->sum('opening_balance')-$contact->sum('opening_balance_paid');
      
        $output['total_purchase_inc_tax'] = $purchase_details->sum('final_total');
        //$output['total_purchase_exc_tax'] = $purchase_details->sum('total_exc_tax');
        $output['total_purchase_exc_tax'] = $purchase_details->sum('total_before_tax');
        $output['purchase_due'] = $purchase_details->sum('final_total') -
                                    $purchase_details->sum('total_paid')+ $contacts_balance_due;
        $output['total_shipping_charges'] = $purchase_details->sum('shipping_charges');
     ...
     * @return array
     */
    public function getOpenSellTotals($business_id, $start_date = null, $end_date = null, $location_id = null, $created_by = null)
    {      $contacts = Contact::where('contacts.business_id', $business_id)
        ...
        if (!empty($created_by)) {
            $query->where('transactions.created_by', $created_by);
            $contacts->where('transactions.created_by', $created_by);
        }
        $contact=$contacts->get();
        $sell_details = $query->get();
        $contacts_balance_due=$contact->sum('opening_balance')-$contact->sum('opening_balance_paid');
        ...
        //$output['total_sell_exc_tax'] = $sell_details->sum('total_exc_tax');
        $output['total_sell_exc_tax'] = $sell_details->sum('total_before_tax');
        $output['invoice_due'] = $sell_details->sum('final_total') - $sell_details->sum('total_paid') + $contacts_balance_due;
        $output['total_shipping_charges'] = $sell_details->sum('shipping_charges');
    ...
    * @return object
    */
   public function mapPurchaseSell($business, $transaction_lines, $mapping_type = 'lab_purchase', $check_expiry = true, $purchase_line_id = null)
   {
       ...
       foreach ($transaction_lines as $line) {
           ...
           $query = Transaction::join('purchase_lines AS PL', 'transactions.id', '=', 'PL.transaction_id')
               ...
               ->whereIn('transactions.type', ['lab_purchase', 'purchase_transfer',
                   'lab_opening_stock', 'production_purchase'])
           ...
           foreach ($rows as $k => $row) {
            ...
            //    dd($row->quantity_adjusted);
               ...
               if ($mapping_type == 'lab_stock_adjustment') {
               ...
               } elseif ($mapping_type == 'lab_purchase') {
                   //Mapping of purchase
           ...
           if (! ($qty_selling == 0 || is_null($qty_selling))) {
               ...
               if (!$allow_overselling) {
                   ...
                   } elseif ($mapping_type == 'lab_stock_adjustment') {
                       $mismatch_error = trans(
    ...
    public function adjustMappingPurchaseSell(
        ...
        if ($status_before == 'final' && $transaction->status == 'draft') {
            ...
            $this->mapPurchaseSell($business, $transaction->sell_lines, 'lab_purchase');
    ...
    public function adjustMappingPurchaseSellAfterEditingPurchase($before_status, $transaction, $delete_purchase_lines)
    {
        ...
        if (!empty($sell_lines)) {
            ...
            $this->mapPurchaseSell($business, $sell_lines, 'lab_purchase');
        }
        ...
        if (!empty($stock_adjustment_lines)) {
            ...
            $this->mapPurchaseSell($business, $stock_adjustment_lines, 'lab_stock_adjustment');
        }
    }
    ...
    */
    ...
    public function getTransactionTotals(
        ...
        if (in_array('lab_purchase_return', $transaction_types)) {
            $query->addSelect(
                DB::raw("SUM(IF(transactions.type='lab_purchase_return', final_total, 0)) as total_purchase_return_inc_tax"),
                DB::raw("SUM(IF(transactions.type='lab_purchase_return', total_before_tax, 0)) as total_purchase_return_exc_tax")
            );
        }

        if (in_array('lab_sell_return', $transaction_types)) {
            $query->addSelect(
                DB::raw("SUM(IF(transactions.type='lab_sell_return', final_total, 0)) as total_sell_return_inc_tax"),
                DB::raw("SUM(IF(transactions.type='lab_sell_return', total_before_tax, 0)) as total_sell_return_exc_tax")
            );
        }

        if (in_array('lab_sell_transfer', $transaction_types)) {
            $query->addSelect(
                DB::raw("SUM(IF(transactions.type='lab_sell_transfer', shipping_charges, 0)) as total_transfer_shipping_charges")
                
            );
        }

        if (in_array('lab_expense', $transaction_types)) {
            $query->addSelect(
                DB::raw("SUM(IF(transactions.type='lab_expense', final_total, 0)) as total_expense")
            );
        }

        if (in_array('doctorexpense', $transaction_types)) {
            $query->addSelect(
                DB::raw("SUM(IF(transactions.type='doctorexpense', final_total, 0)) as doctor_commission")
            );
        }

        if (in_array('lab_stock_adjustment', $transaction_types)) {
            $query->addSelect(
                DB::raw("SUM(IF(transactions.type='lab_stock_adjustment', final_total, 0)) as total_adjustment"),
                DB::raw("SUM(IF(transactions.type='lab_stock_adjustment', total_amount_recovered, 0)) as total_recovered")
            );
        }

        if (in_array('lab_purchase', $transaction_types)) {
            $query->addSelect(
                DB::raw("SUM(IF(transactions.type='lab_purchase', IF(discount_type = 'percentage', COALESCE(discount_amount, 0)*total_before_tax/100, COALESCE(discount_amount, 0)), 0)) as total_purchase_discount")
            );
        }

        if (in_array('lab_sell', $transaction_types)) {
            $query->addSelect(
                DB::raw("SUM(IF(transactions.type='lab_sell' AND transactions.status='final', IF(discount_type = 'percentage', COALESCE(discount_amount, 0)*total_before_tax/100, COALESCE(discount_amount, 0)), 0)) as total_sell_discount"),
                DB::raw("SUM(IF(transactions.type='lab_sell' AND transactions.status='final', rp_redeemed_amount, 0)) as total_reward_amount"),
                DB::raw("SUM(IF(transactions.type='lab_sell' AND transactions.status='final', round_off_amount, 0)) as total_sell_round_off")
            );
        }
        ...
        if (in_array('doctorexpense', $transaction_types)) {
            $output['doctor_commission'] =
                !empty($transaction_totals->doctor_commission) ?
                $transaction_totals->doctor_commission : 0;
        }
    ...
    public function getGrossProfit($business_id, $start_date = null, $end_date = null, $location_id = null)
    {
        ...
        //             ->join('test_sells as ts', 'ts.id', '=', 'sale.id')
        //             ->where('sale.business_id', $business_id);
        $query = TestSell::join('transactions as sale', 'test_sells.transaction_id', '=', 'sale.id');
        ...
        //             ->first();
        $gross_profit_obj= $query->select(
            
            DB::raw('SUM(test_sells.final_total - test_sells.test_cast_amount ) as  gross_profit'),
            DB::raw('SUM(test_sells.more_amount) as  total_cost')
        ...
        $transaction_totals = $this->labgetTransactionTotals($business_id, ['lab_sell'], $start_date, $end_date, $location_id);
    ...
    * @return array
    */
   public function labdeleteSale($business_id, $transaction_id)
   {
       //Check if return exist then not allowed
       if ($this->isReturnExist($transaction_id)) {
           $output = [
    ...
    /**
     * Add/Edit Test sell lines
     *
     * @param object/int $transaction
     * @param array $test
     * @return boolean/object
     */
    public function createTestSellLines($transaction,$test,$doctor_id)
    {      
           foreach ($test as $key => $value) {
            $ref_count = $this->commonUtil->setAndGetReferenceCount('report');
            //Generate reference number
            $report_id ='RP'.$this->commonUtil->generateReferenceNumber(' report', $ref_count);
            $test_data = [
                'test_id'=>$value['id'],
                'transaction_id'=>$transaction->id,
                'patient_id'=>$transaction->contact_id,
                ...
                'test_cast_amount'=>$value['test_cast_amount'],
                'more_amount' =>$value['more_amount'],
                'final_total'=>$value['final_total']
                 ];
                

                $test_sell=TestSell::create($test_data);

                $heads=TestHead::where('test_id',$value['id'])->get();
               
                foreach ($heads as $key => $head) {
                   
               
                $report_head=ReportHead::where('id',$head['report_head_id'])->get();
               
                foreach ($report_head as $key =>$reporthead) {
                   
                $test_particular=TestParticular::where('report_head_id',$reporthead->id)->get();
                foreach ($test_particular as $key => $values) {
                    $particular= [
                        "report_head_id"=>$values->report_head_id,
                        "name" => $values->name,
                        "result" => $values['result'],
                        "test_sell_id"=>$test_sell->id,
                        "unit" => $values['unit'],
                        'male'=> $values['male'],
                  ...
                  $ReportTestParticular=ReportTestParticular::create($particular);
                }
     ...
     * @return boolean/object
     */
    public function UpdateTestSellLines($transaction,$test,$doctor_id)
    {        $test_sell_delete=TestSell::where('transaction_id',$transaction->id)->delete();
     ...
     * @return array
     */
    public function testgetReceiptDetails($transaction_id, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type)
    {
        $il = $invoice_layout;
        ...
        $output['lines'] = [];
        if ($transaction_type == 'lab_sell') {
        
            $lines = $transaction->test_sell()->with(['tests'])->get();
            $output['lines'] =$lines;
   ...
   //Subtotal
   $output['subtotal_label'] = $il->sub_total_label . ':';
   $output['subtotal'] = ($transaction->total_before_tax + $discount  != 0) ? $this->num_f($transaction->total_before_tax + $discount , $show_currency, $business_details) : 0;
   $output['subtotal_unformatted'] = ($transaction->total_before_tax != 0) ? $transaction->total_before_tax : 0;
        ...
        //Paid & Amount due, only if final
        if ($transaction_type == 'lab_sell' && $transaction->status == 'final') {
            $paid_amount = $this->getTotalPaid($transaction->id);
            $due = $transaction->final_total - $paid_amount;
     ...
     * @return array
     */
    public function labgetSellTotals($business_id, $start_date = null, $end_date = null, $location_id = null, $created_by = null)
    {   
     ...
     * @return array
     */
    public function labgetOpenPurchaseTotals($business_id, $start_date = null, $end_date = null, $location_id = null)
    {
        $contacts = Contact::where('contacts.business_id', $business_id)
        ->where('contacts.type', 'supplier')
        ->join('transactions AS t', 'contacts.id', '=', 't.contact_id')
     ...
     * @return array
     */
    public function labgetTransactionTotals(
        $business_id,
        $transaction_types,
        $start_date = null,
        $end_date = null,
    ...
    * @return Obj
    */
    public function labgetSellsLast30Days($business_id, $group_by_location = false)
    {
        $query = Transaction::leftjoin('transactions as SR', function ($join) {
            $join->on('SR.return_parent_id', '=', 'transactions.id')
                                    ->where('SR.type', 'sell_return');
        ...
        $query->where('transactions.location_id', 2);
        $query->select(
            DB::raw("DATE_FORMAT(transactions.transaction_date, '%Y-%m-%d') as date"),
            DB::raw("SUM( transactions.final_total - COALESCE(SR.final_total, 0) ) as total_sells")
        )
