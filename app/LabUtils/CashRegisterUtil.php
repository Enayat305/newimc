...
class CashRegisterUtil extends Util
{
     ...
     * @return boolean
     */
    public function labaddSellPayments($transaction, $payments)
    {
        $user_id = auth()->user()->id;
   ...
   /**
     ...
     * @return boolean
     */
    public function lab_updateSellPayments($status_before, $transaction, $payments)
    {
        $user_id = auth()->user()->id;
        $register =  CashRegister::where('user_id', $user_id)
                                ->where('status', 'open')
    ...
    {   
        ...
        $register_details = $query->select(
            ...
            DB::raw("SUM(IF(transaction_type='lab_sell', amount, IF(transaction_type='refund', -1 * amount, 0))) as total_sale"),
            DB::raw("SUM(IF(pay_method='cash', IF(transaction_type='lab_sell', amount, 0), 0)) as total_cash"),
            DB::raw("SUM(IF(pay_method='cheque', IF(transaction_type='lab_sell', amount, 0), 0)) as total_cheque"),
            DB::raw("SUM(IF(pay_method='card', IF(transaction_type='lab_sell', amount, 0), 0)) as total_card"),
            DB::raw("SUM(IF(pay_method='bank_transfer', IF(transaction_type='lab_sell', amount, 0), 0)) as total_bank_transfer"),
            DB::raw("SUM(IF(pay_method='other', IF(transaction_type='lab_sell', amount, 0), 0)) as total_other"),
            DB::raw("SUM(IF(pay_method='custom_pay_1', IF(transaction_type='lab_sell', amount, 0), 0)) as total_custom_pay_1"),
            DB::raw("SUM(IF(pay_method='custom_pay_2', IF(transaction_type='lab_sell', amount, 0), 0)) as total_custom_pay_2"),
            DB::raw("SUM(IF(pay_method='custom_pay_3', IF(transaction_type='lab_sell', amount, 0), 0)) as total_custom_pay_3"),
     ...
     * @return object
     */
    public function getCloseRegisterDetails($register_id = null)
    {
        $query = CashRegister::join(
            'cash_register_transactions as ct',
            'ct.cash_register_id',
    ...
    {   
        $product_details = Transaction::where('transactions.created_by', $user_id)
                ...
                ->select(
                    ...
                    DB::raw('SUM(TSL.final_total) as total_amount')
                )
