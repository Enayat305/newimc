...
class LaboratoryReportController extends Controller
{
    ...
    public function getProfitLoss(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            //Get Sell details
            $sell_details = $this->transactionUtil->labgetSellTotals(
                $business_id,
                $start_date,
                $end_date,
                $location_id
            ...
            $transaction_types = [
                'lab_purchase_return', 'lab_sell_return','doctorexpense', 'lab_expense', 'lab_stock_adjustment', 'lab_sell_transfer', 'lab_purchase', 'lab_sell'
            ];
            ...
            $data['doctor_commission'] =  $transaction_totals['doctor_commission']; 
            //Stock adjustments
            ...
            return view('Laboratory/report.partials.profit_loss_details', compact('data'))->render();
        }

        $business_locations = BusinessLocation::labforDropdown($business_id, true);
        return view('Laboratory/report.profit_loss', compact('business_locations'));
    }
    ...
    public function getPurchaseSell(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $transaction_types = [
                'lab_purchase_return', 'lab_sell_return'
            ];
        ...
        return view('Laboratory/report.purchase_sell')
    ...
    public function getCustomerSuppliers(Request $request)
    {
        ...
        if ($request->ajax()) {
            $contacts = Contact::where('contacts.business_id', $business_id)
                ...
                ->select(
                    ...
                    DB::raw("SUM(IF(t.type = 'lab_purchase_return', (SELECT SUM(amount) FROM transaction_payments WHERE transaction_payments.transaction_id=t.id), 0)) as purchase_return_received"),
        ...
        return view('Laboratory/report.contact')
    ...
    public function getStockReport(Request $request)
    {
        ...
        foreach ($selling_price_groups as $selling_price_group) {
            if (auth()->user()->can('lab_selling_price_group.' . $selling_price_group->id)) {
                $allowed_selling_price_group = true;
                break;
            }
        }

        if ($this->moduleUtil->isModuleInstalled('Manufacturing') && (auth()->user()->can('lab_superadmin') || $this->moduleUtil->hasThePermissionInSubscription($business_id, 'manufacturing_module'))) {
        ...
        if ($request->ajax()) {
            ...
            $products = $query->select(
                ...
                DB::raw("(SELECT SUM(TSL.quantity - TSL.quantity_returned) FROM transactions 
                        ...
                        WHERE transactions.status='final' AND transactions.type='lab_sell' $location_filter 
                        AND TSL.variation_id=variations.id) as total_sold"),
                DB::raw("(SELECT SUM(IF(transactions.type='lab_sell_transfer', TSL.quantity, 0) ) FROM transactions 
                        JOIN transaction_sell_lines AS TSL ON transactions.id=TSL.transaction_id
                        WHERE transactions.status='final' AND transactions.type='lab_sell_transfer' $location_filter 
                        AND (TSL.variation_id=variations.id)) as total_transfered"),
                DB::raw("(SELECT SUM(IF(transactions.type='lab_stock_adjustment', SAL.quantity, 0) ) FROM transactions 
                        JOIN stock_adjustment_lines AS SAL ON transactions.id=SAL.transaction_id
                        WHERE transactions.status='received' AND transactions.type='lab_stock_adjustment' $location_filter 
            ...
            $datatable =  Datatables::of($products)
                ...
                ->editColumn('unit_price', function ($row) use ($allowed_selling_price_group) {
                    $html = '';
                    if (auth()->user()->can('lab_access_default_selling_price')) {
        ...
        return view('Laboratory/report.stock_report')
    ...
    public function getStockDetails(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $product_details =  $query->select(
                ...
                DB::raw("SUM(vld.qty_available) as stock"),
                DB::raw("(SELECT SUM(IF(transactions.type='lab_sell', TSL.quantity - TSL.quantity_returned, -1* TPL.quantity) ) FROM transactions 
                ...
                DB::raw("(SELECT SUM(IF(transactions.type='lab_sell_transfer', TSL.quantity, 0) ) FROM transactions 
                        LEFT JOIN transaction_sell_lines AS TSL ON transactions.id=TSL.transaction_id
                        WHERE transactions.status='final' AND transactions.type='lab_ell_transfer' $location_filter 
            ...
            return view('Laboratory/report.stock_details')
    ...
    {
        if (!auth()->user()->can('lab_tax_report.view')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        return view('Laboratory/report.tax_report')
                    ->with(compact('business_locations'));
    }
    ...
    public function getTrendingProducts(Request $request)
    {
        ...
        $categories = Category::forDropdown($business_id, 'product');
        $brands = Department::pluck('name', 'id');
        $units = Unit::where('business_id', $business_id)
                            ->pluck('short_name', 'id');
        $business_locations = BusinessLocation::labforDropdown($business_id, true);

        return view('Laboratory/report.trending_products')
    ...
    public function getExpenseReport(Request $request)
    {
        ...
        return view('Laboratory/report.expense_report')
    ...
    public function getStockAdjustmentReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            $query =  Transaction::where('business_id', $business_id)
                            ->where('type', 'lab_stock_adjustment');
        ...
        return view('Laboratory/report.stock_adjustment_report')
                    ->with(compact('business_locations'));
    }
    ...
    public function getRegisterReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $registers->where('cash_registers.location_id', 2);
            return Datatables::of($registers)
                ...
                ->addColumn('action', '<button type="button" data-href="{{action(\'Lab\\CashRegisterController@show\', [$id])}}" class="btn btn-xs btn-info btn-modal" 
        ...
        return view('Laboratory/report.register_report')
                    ->with(compact('users'));
    }
    ...
    public function getStockExpiryReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $report=[];
            foreach($check_report->get() as $key => $reports ){
                if($reports->stock_left == 0){
            ...
            return Datatables::of($report)
                ...
                ->editColumn('ref_no', function ($row) {
                    return '<button type="button" data-href="' . action('Lab\PurchaseController@show', [$row->transaction_id])
        ...
        return view('Laboratory/report.stock_expiry_report')
    ...
    public function getStockExpiryReportEditModal(Request $request, $purchase_line_id)
    {
        ...
        if ($request->ajax()) {
            ...
            return view('Laboratory/report.partials.stock_expiry_edit_modal')
    ...
    public function getCustomerGroup(Request $request)
    {
        ...
        return view('Laboratory/eport.customer_group')
    ...
    public function getproductPurchaseReport(Request $request)
    {
        ...
        return view('Laboratory/report.product_purchase_report')
    ...
    public function getproductSellReport(Request $request)
    {
        ...
        return view('Laboratory/report.product_sell_report')
    ...
    public function getLotReport(Request $request)
    {
        ...
        return view('Laboratory/report.lot_report')
    ...
    public function purchasePaymentReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $query = TransactionPayment::leftjoin('transactions as t', function ($join) use ($business_id) {
                $join->on('transaction_payments.transaction_id', '=', 't.id')
                    ->where('t.business_id', $business_id)
                    ->whereIn('t.type', ['lab_purchase', 'opening_balance']);
            })
                ->where('transaction_payments.business_id', $business_id)
                ->where(function ($q) use ($business_id, $contact_filter1, $contact_filter2, $parent_payment_query_part) {
                    $q->whereRaw("(transaction_payments.transaction_id IS NOT NULL AND t.type IN ('lab_purchase', 'opening_balance')  $parent_payment_query_part $contact_filter1)")
                        ->orWhereRaw("EXISTS(SELECT * FROM transaction_payments as tp JOIN transactions ON tp.transaction_id = transactions.id WHERE transactions.type IN ('lab_purchase', 'opening_balance') AND transactions.business_id = $business_id AND tp.parent_id=transaction_payments.id $contact_filter2)");
                })
            ...
            return Datatables::of($query)
                 ->editColumn('ref_no', function ($row) {
                     if (!empty($row->ref_no)) {
                         return '<a data-href="' . action('Lab\PurchaseController@show', [$row->transaction_id])
                            . '" href="#" data-container=".view_modal" class="btn-modal">' . $row->ref_no . '</a>';
                     } else {
                         return '';
                     }
                ...
                ->addColumn('action', '<button type="button" class="btn btn-primary btn-xs view_payment" data-href="{{ action("Lab\TransactionPaymentController@viewPayment", [$DT_RowId]) }}">@lang("messages.view")
        ...
        return view('Laboratory/report.purchase_payment_report')
            ->with(compact('business_locations', 'suppliers'));
    }
    ...
    public function sellPaymentReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $query = TransactionPayment::leftjoin('transactions as t', function ($join) use ($business_id) {
                $join->on('transaction_payments.transaction_id', '=', 't.id')
                    ->where('t.business_id', $business_id)
                    ->whereIn('t.type', ['lab_sell', 'opening_balance']);
            })
                ->leftjoin('contacts as c', 't.contact_id', '=', 'c.id')
                ->where('transaction_payments.business_id', $business_id)
                ->where(function ($q) use ($business_id, $contact_filter1, $contact_filter2, $parent_payment_query_part) {
                    $q->whereRaw("(transaction_payments.transaction_id IS NOT NULL AND t.type IN ('lab_sell', 'opening_balance') $parent_payment_query_part $contact_filter1)")
                        ->orWhereRaw("EXISTS(SELECT * FROM transaction_payments as tp JOIN transactions ON tp.transaction_id = transactions.id WHERE transactions.type IN ('lab_sell', 'opening_balance') AND transactions.business_id = $business_id AND tp.parent_id=transaction_payments.id $contact_filter2)");
                })
                ->select(
                    DB::raw("IF(transaction_payments.transaction_id IS NULL, 
                                (SELECT c.name FROM transactions as ts
            ...
            return Datatables::of($query)
                 ->editColumn('invoice_no', function ($row) {
                     if (!empty($row->transaction_id)) {
                         return '<a data-href="' . action('Lab\SellController@show', [$row->transaction_id])
                            . '" href="#" data-container=".view_modal" class="btn-modal">' . $row->invoice_no . '</a>';
                     } else {
                         return '';
                     }
        ...
        return view('Laboratory/report.sell_payment_report')
    ...
    public function getTableReport(Request $request)
    {
        ...
        return view('Laboratory/report.table_report')
            ->with(compact('business_locations'));
    }
    ...
    public function getServiceStaffReport(Request $request)
    {
        ...
        return view('Laboratory/report.service_staff_report')
    ...
    {
        if (!auth()->user()->can('lab_report.stock_details')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        if (!empty(request()->input('location_id'))) {
            ...
            $stock_details = $query->select(
                DB::raw("(SELECT SUM(COALESCE(TSL.quantity, 0)) FROM transactions 
                        LEFT JOIN transaction_sell_lines AS TSL ON transactions.id=TSL.transaction_id
                        WHERE transactions.status='final' AND transactions.type='lab_sell' AND transactions.location_id=$location_id 
                        AND TSL.variation_id=variations.id) as total_sold"),
                ...
                DB::raw("(SELECT SUM(COALESCE(TSL.quantity,0)) FROM transactions 
                        LEFT JOIN transaction_sell_lines AS TSL ON transactions.id=TSL.transaction_id
                        WHERE transactions.status='final' AND transactions.type='lab_sell_transfer' AND transactions.location_id=$location_id 
                ...
                DB::raw("(SELECT SUM(COALESCE(PL.quantity,0)) FROM transactions 
                        ...
                        WHERE transactions.status='received' AND transactions.type='lab_purchase_transfer' AND transactions.location_id=$location_id 
                ...
                DB::raw("(SELECT SUM(COALESCE(SAL.quantity, 0)) FROM transactions 
                        LEFT JOIN stock_adjustment_lines AS SAL ON transactions.id=SAL.transaction_id
                        WHERE transactions.status='received' AND transactions.type='lab_stock_adjustment' AND transactions.location_id=$location_id 
                        AND SAL.variation_id=variations.id) as total_adjusted"),
                DB::raw("(SELECT SUM(COALESCE(PL.quantity, 0)) FROM transactions 
                        LEFT JOIN purchase_lines AS PL ON transactions.id=PL.transaction_id
                        WHERE transactions.status='received' AND transactions.type='lab_purchase' AND transactions.location_id=$location_id
        ...
        $business_locations = BusinessLocation::labforDropdown($business_id);
        return view('Laboratory/report.product_stock_details')
    ...
    {
        $business_id = request()->session()->get('user.business_id');
        $query = Transaction::where('transactions.type', 'lab_sell')
        ->where('transactions.status', 'final')
        ->join('test_sells AS TSL', 'transactions.id', '=', 'TSL.transaction_id')
        ->join('tests AS P', 'TSL.test_id', '=', 'P.id')
        ->select(
            'P.name as test_name',
            DB::raw('Count(P.name) as total_quantity'),
            DB::raw('SUM(TSL.final_total - TSL.test_cast_amount) as gross_profit')
        );
    ...
    //     (TSPL.quantity - TSPL.qty_returned) * (transaction_sell_lines.unit_price_inc_tax - PL.purchase_price_inc_tax)) )) AS gross_profit'));
        ...
        if ($by == 'brand') {
            $query->leftJoin('departments as B', 'B.id', '=', 'P.department_id')
                ->addSelect("B.name as brand")
                ->groupBy('B.id');
        }
       
        if ($by == 'location') {
            $query->join('business_locations as L', 'transactions.location_id', '=', 'L.id')
                ->addSelect("L.name as location")
                ->groupBy('L.id');
        }

        if ($by == 'invoice') {
            $query->addSelect('transactions.invoice_no', 'transactions.id as transaction_id')
                ->groupBy('transactions.invoice_no');
        }

        if ($by == 'date') {
            $query->addSelect("transactions.transaction_date")
                ->groupBy(DB::raw('DATE(transactions.transaction_date)'));
        }

        if ($by == 'day') {
            $results = $query->addSelect(DB::raw("DAYNAME(transactions.transaction_date) as day"))
        ...
        if ($by == 'customer') {
            $query->join('contacts as CU', 'transactions.contact_id', '=', 'CU.id')
    ...
    public function itemsReport()
    {
        ...
        if (request()->ajax()) {
            ...
            return Datatables::of($query)
                ...
                ->editColumn('purchase_ref_no', function ($row) {
                    $html = $row->purchase_type == 'lab_purchase' ? '<a data-href="' . action('Lab\PurchaseController@show', [$row->purchase_line_id])
        ...
        $customers = Contact::customersDropdown($business_id, false);
        $business_locations = BusinessLocation::labforDropdown($business_id);
        return view('Laboratory/report.items_report')->with(compact('suppliers', 'customers', 'business_locations'));
    }
}
