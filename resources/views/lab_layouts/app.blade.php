...
@if($request->segment(1) == 'los' && ($request->segment(2) == 'create' || $request->segment(3) == 'edit'))
    @php
        $pos_layout = true;
        $poss_layout = true;
    @endphp
@elseif($request->segment(2) == 'multi' && ($request->segment(3) == 'create' || $request->segment(3) == 'edit'))
@php
    $pos_layout = true;
    $poss_layout = false;
@endphp
...
<html lang="{{ app()->getLocale() }}" dir="{{in_array(session()->get('user.language', config('app.locale')), config('constants.langs_rtl')) ? 'rtl' : 'ltr'}}">
    <head>
        ...
        @include('lab_layouts.partials.css')
    ...
    <body class="@if($pos_layout) hold-transition lockscreen @else hold-transition skin-@if(!empty(session('business.theme_color'))){{session('business.theme_color')}}@else{{'blue-light'}}@endif sidebar-mini @endif">
        ...
        <div class="wrapper thetop">
            ...
            @if(!$pos_layout)
                @include('lab_layouts.partials.header')
                @include('lab_layouts.partials.sidebar')
            @elseif($poss_layout== false)
            @else
                 @include('lab_layouts.partials.header-pos')
            ...
            @if(!$pos_layout)
                @include('lab_layouts.partials.footer')
            @else
                @include('lab_layouts.partials.footer_pos')
            @endif
        ...
        @include('lab_layouts.partials.javascripts')
