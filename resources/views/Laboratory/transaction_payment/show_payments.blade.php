    ...
    <div class="modal-content">
        <div class="modal-header">
            ...
            <h4 class="modal-title no-print">
                ...
                (
                @if(in_array($transaction->type, ['lab_purchase','doctorexpense', 'lab_expense', 'lab_purchase_return', 'payroll']))    
                    @lang('purchase.ref_no'): {{ $transaction->ref_no }} 
                @elseif(in_array($transaction->type, ['lab_sell', 'lab_sell_return']))
            ...
            <h4 class="modal-title visible-print-block">
                @if(in_array($transaction->type, ['lab_purchase', 'doctorexpense','lab_expense', 'lab_purchase_return', 'payroll'])) 
                    @lang('purchase.ref_no'): {{ $transaction->ref_no }}
                @elseif($transaction->type == 'lab_sell')
                    @lang('sale.invoice_no'): {{ $transaction->invoice_no }}
                @endif
            </h4>
        </div>
        ...
        <div class="modal-body">
            @if(in_array($transaction->type, ['lab_purchase', 'lab_purchase_return']))
                <div class="row invoice-info">
                    <div class="col-sm-4 invoice-col">
                        @include('Laboratory/transaction_payment.transaction_supplier_details')
                    </div>
                    <div class="col-md-4 invoice-col">
                        @include('Laboratory/transaction_payment.payment_business_details')
                    </div>
            ...
            @elseif($transaction->type == 'lab_expense' || $transaction->type == 'doctorexpense')
                <div class="row invoice-info">
            ...
            @can('send_notification')
                @if($transaction->type == 'lab_purchase')
            ...
            @if($transaction->payment_status != 'paid')
                <div class="row">
                    <div class="col-md-12">
                        @if((auth()->user()->can('purchase.payments') && (in_array($transaction->type, ['lab_purchase', 'lab_purchase_return']) )) || (auth()->user()->can('sell.payments') && (in_array($transaction->type, ['lab_sell', 'lab_sell_return']))) || (auth()->user()->can('expense.access') ) )
                            <a href="{{ action('Lab\TransactionPaymentController@addPayment', [$transaction->id]) }}" class="btn btn-primary btn-xs pull-right add_payment_modal no-print"><i class="fa fa-plus" aria-hidden="true"></i> @lang("purchase.add_payment")</a>
                        @endif
                    </div>
                </div>
            @endif
                        ...
                        @forelse ($payments as $payment)
                            <tr>
                              ...
                              @if((auth()->user()->can('purchase.payments') && (in_array($transaction->type, ['lab_purchase', 'lab_purchase_return']) )) || (auth()->user()->can('sell.payments') && (in_array($transaction->type, ['lab_sell', 'lab_sell_return']))) || auth()->user()->can('expense.access') )
                                ...
                                data-href="{{action('Lab\TransactionPaymentController@edit', [$payment->id]) }}"><i class="glyphicon glyphicon-edit"></i></button>
                                &nbsp; <button type="button" class="btn btn-danger btn-xs delete_payment" 
                                data-href="{{ action('Lab\TransactionPaymentController@destroy', [$payment->id]) }}"
                                ><i class="fa fa-trash" aria-hidden="true"></i></button>
                                &nbsp;
                                <button type="button" class="btn btn-primary btn-xs view_payment" data-href="{{ action('Lab\TransactionPaymentController@viewPayment', [$payment->id]) }}">
