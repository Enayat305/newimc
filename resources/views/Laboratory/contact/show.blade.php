            ...
            <div class="nav-tabs-custom">
                ...
                <div class="tab-content">
                    <div class="tab-pane
                            ...
                            @endif"
                            id="contact_info_tab">
                        @include('Laboratory/contact.partials.contact_info_tab')
                    </div>
                    <div class="tab-pane
                                @if(!empty($view_type) &&  $view_type == 'ledger')
                                    active
                                ...
                                @endif"
                            id="ledger_tab">
                        @include('Laboratory/contact.partials.ledger_tab')
                    </div>
                    @if(in_array($contact->type, ['both', 'supplier']))
                        <div class="tab-pane
                            @if(!empty($view_type) &&  $view_type == 'purchase')
                        ...
                        <div class="tab-pane 
                            ...
                            @include('Laboratory/contact.partials.stock_report_tab')
                        </div>
                    @endif
                    @if(in_array($contact->type, ['both', 'customer']))
                        <div class="tab-pane 
                    ...
                    <div class="tab-pane
                        ...
                        @include('Laboratory/contact.partials.documents_and_notes_tab')
                    </div>
                </div>
            </div>
        </div>
...
$(document).ready( function(){
    ...
    rp_log_table = $('#rp_log_table').DataTable({
        ...
        serverSide: true,
        aaSorting: [[0, 'desc']],
        ajax: '/lab/sells?customer_id={{ $contact->id }}&rewards_only=true',
        columns: [
    ...
    supplier_stock_report_table = $('#supplier_stock_report_table').DataTable({
        ...
        serverSide: true,
        'ajax': {
            url: "{{action('Lab\ContactController@getSupplierStockReport', [$contact->id])}}",
            data: function (d) {
    ...
    $('#contact_id').change( function() {
        if ($(this).val()) {
            window.location = "{{url('/lab/contacts')}}/" + $(this).val();
        }
    });
});
...
function get_contact_ledger() {
    ...
    $.ajax({
        url: '/lab/contacts/ledger?contact_id={{$contact->id}}&start_date=' + start_date + '&transaction_types=' + transaction_types + '&show_payments=' + show_payments + '&end_date=' + end_date,
        dataType: 'html',
