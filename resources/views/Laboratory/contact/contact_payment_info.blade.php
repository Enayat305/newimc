...
@if(!empty($contact->opening_balance) && $contact->opening_balance != '0.00')
    ...
    @if ($contact->type == 'supplier' || $contact->type == 'both')
    <strong>@lang('Total All Due')</strong>
    <p class="text-muted">
    <span class="display_currency" data-currency_symbol="true">
       
        {{ ($contact->opening_balance - $contact->opening_balance_paid)+ ($contact->total_purchase - $contact->purchase_paid )}}
    </span>
    </p>
    @else
    <strong>@lang('Total All Due')</strong>
    <p class="text-muted">
    <span class="display_currency" data-currency_symbol="true">
        {{ ($contact->opening_balance - $contact->opening_balance_paid)+($contact->total_invoice - $contact->invoice_received) }}
