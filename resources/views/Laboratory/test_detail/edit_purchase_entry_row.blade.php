    ...
    @foreach($test->use_product_tests as $use_product_tests)
        <tr>
            ...
            <td>
                ...
                @if(!empty( $use_product_tests->sub_unit_id))
                <br>
                <select name="purchases[{{$loop->index}}][sub_unit_id]" class="form-control input-sm sub_unit">
                    
                        <option value="{{$use_product_tests->sub_unit_id}}" data-multiplier="{{$use_product_tests->product->multiplier}}">
                           {{$use_product_tests->product->unit->short_name}}
                        </option>
                   
                </select>
                {{-- <input type="text" name="purchases[{{$row_count}}][multiplier]" value="{{$value['multiplier']}}"> --}}
            ...
            <td class="{{$hide_tax}} hidden">
                ...
                {!! Form::text('purchases[' . $row_count . '][purchase_price_inc_tax]', $use_product_tests->purchase_price, ['class' => 'form-control input-sm purchase_unit_cost_after_tax input_number', 'required']); !!}
            </td>
